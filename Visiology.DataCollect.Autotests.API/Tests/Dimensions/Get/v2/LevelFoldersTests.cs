...
namespace Visiology.DataCollect.Autotests.API.Tests.Dimensions.Get.v2
{
    ...
    public class LevelFoldersTests : BaseTests<FoldersListDto, FolderDto>
    {
        ...
        {
            Url = string.Format($"{config.GetValue("ApiUrl")}{config.GetValue("ApiUrlGetDimensionFolderChildrenPath")}", dimensionId, levelId);
        ...
        {
            return string.Format($"{config.GetValue("ApiUrl")}{config.GetValue("ApiUrlGetDimensionLevelFoldersPath")}", dimensionId, levelId);
        }
    }
}
