...
{
    /// <summary>
    /// Класс тестирования получения уровней измерения
    /// </summary>
    [XApiVersion("2.0")]
    public class LevelFoldersTests : BaseTests<FoldersListDto, FolderDto>
    {
        ...
        /// В дампе - "Измерение для тестирования получения папок"
        /// </summary>
        private const string levelId = "level_Uroven_2";
        ...
        /// <summary>
        /// Тест получения дочерних каталогов при невалидном или несуществующем идентификаторе уровня
        /// </summary>
        ...
        [InlineData(null)]
        public async Task GetAll_WithInvalidDimensionId(string levelId)
        {
            this.Url = string.Format($"{this.config.GetValue("ApiUrl")}{this.config.GetValue("ApiUrlGetDimensionFolderChildrenPath")}", dimensionId, levelId);
        ...
        {
            return string.Format($"{this.config.GetValue("ApiUrl")}{this.config.GetValue("ApiUrlGetDimensionLevelFoldersPath")}", dimensionId, levelId);
        }
    }
}
